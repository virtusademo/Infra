...
cat << EOF > /opt/apache-tomcat-7.0.77/conf/tomcat-users.xml
...
if [ -f "/opt/apache-tomcat-7.0.77/webapps/manager/WEB-INF/web.xml" ]
then
...
exec /opt/apache-tomcat-7.0.77/bin/catalina.sh run
